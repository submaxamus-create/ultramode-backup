...
# 2026-03-22 00:00 | ChatGPT | v9.5    | r1 | Style-pass only: pushed UM closer to umpatcher visual language with stronger title bars, boxed row selection, tighter footer hierarchy, sharper active-panel emphasis, and preserved desktop/buffer behavior.
# =============================================================================
# ### ULTRAMODE PROTECTED START
# =============================================================================
...
WINDOW_TITLE = "ULTRAMODE MAIN MENU v9.5"
...
logger.info("ultramode starting (%s r1) via %s", "v9.5", GAMEPAD_SOURCE)
...
def rounded_panel(rect, title=None, fill_key="PANEL", border_key="BORDER", title_color=None):
    ...
    if title:
        ...
        line_y = rect.y + 30
        pygame.draw.line(screen, COLORS[border_key], (rect.x + 12, line_y), (rect.right - 12, line_y), 1)
...
def grid_cell(rect, label, value, value_color=None):
    ...
    value_lines = wrap_text(str(value), tiny, rect.width - 18)
    screen.blit(label_surf, (rect.x + 9, rect.y + 8))
    for idx, line in enumerate(value_lines[:2]):
        value_surf = tiny.render(line, True, value_color or COLORS["WHITE"])
        screen.blit(value_surf, (rect.x + 9, rect.y + 28 + idx * 14))


def footer_bubble(rect, label, text, label_color=None, text_color=None, fill_key="PANEL_ALT", border_key="BORDER"):
    rounded_box(rect, fill_key=fill_key, border_key=border_key, radius=14, shadow=False)
    label_surf = tiny_bold.render(label, True, label_color or COLORS["NEON"])
    text_lines = wrap_text(str(text), tiny, rect.width - 108)
    screen.blit(label_surf, (rect.x + 12, rect.y + 8))
    screen.blit(tiny.render(text_lines[0], True, text_color or COLORS["TEXT"]), (rect.x + 96, rect.y + 8))
...
def draw_header():
    rect = pygame.Rect(20, 16, WIDTH - 40, 54)
    rounded_panel(rect, fill_key="HEADER", border_key="HEADER_LINE")
    title_surf = small_bold.render(WINDOW_TITLE, True, COLORS["TITLE"])
    clock_surf = tiny.render(get_clock_text(), True, COLORS["NEON"])
    path_surf = tiny.render("/userdata/system/ultramode/um.py", True, COLORS["DIM"])
    screen.blit(title_surf, (rect.x + 14, rect.y + 8))
    screen.blit(path_surf, (rect.x + 14, rect.y + 30))
    screen.blit(clock_surf, (rect.right - clock_surf.get_width() - 14, rect.y + 18))


def draw_top_grid():
    scan = get_desktop_scan()
    buffer_detail = get_buffer_check()
    area = pygame.Rect(20, 82, WIDTH - 40, 84)
    rounded_panel(area, title="LIVE STATUS GRID", fill_key="PANEL_ALT")
    inner_x = area.x + 10
    inner_y = area.y + 18
    gap = 8
...
def draw_list_box(rect, title, items, selected_idx, active, kind="main"):
    ...
    for idx, item in enumerate(items):
        ...
        row_rect = pygame.Rect(row_left, base_y + idx * line_h - 3, row_width, 30)
        if active_item:
            rounded_box(row_rect, fill_key="SELECT_BG", border_key="NEON", radius=10, shadow=False)
            pygame.draw.line(screen, COLORS["TITLE"], (row_rect.right - 18, row_rect.y + 7), (row_rect.right - 18, row_rect.bottom - 7), 2)
            color = COLORS["SELECT"]
...
def draw_detail_panel():
    ...
    for idx, (label, value, color) in enumerate(lines):
        if idx:
            pygame.draw.line(screen, COLORS["BORDER_SOFT"], (rect.x + 14, y - 8), (rect.right - 14, y - 8), 1)
...
def draw_status_bubble():
    ...
    if any(token in lower for token in ("failed", "missing", "error")):
        color = COLORS["RED"]
    footer_bubble(rect, "STATUS", status_message, label_color=COLORS["NEON"], text_color=color, fill_key="PANEL", border_key="NEON")


def draw_help_bubble():
    rect = pygame.Rect(20, 596, WIDTH - 40, 28)
    footer_bubble(rect, "HELP", HELP_TEXT.get(current_item(), "No help text yet."), fill_key="PANEL_ALT", border_key="BORDER")


def draw_ctrl_bubble():
    rect = pygame.Rect(20, 632, WIDTH - 40, 28)
    footer_bubble(rect, "CTRL", "UP/DOWN move   LEFT/RIGHT switch box   ENTER/A select   ESC/B back/exit", fill_key="PANEL_ALT", border_key="BORDER_SOFT")


def draw_path_bubble():
    rect = pygame.Rect(20, 668, WIDTH - 40, 28)
    footer_bubble(rect, "PATH", "/userdata/system/ultramode/um.py", text_color=COLORS["DIM"], fill_key="PANEL_ALT", border_key="BORDER_SOFT")
